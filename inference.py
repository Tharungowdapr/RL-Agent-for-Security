...
"""
    ...
    return Action(action_type="prioritize", target_id=cve_id)


def run_task(task):
    env = SecurityEnv(task)
    obs = env.reset()
    ...
    try:
        while True:
            ...
            action = get_action(obs, env.selected)
            obs, reward, done, info = env.step(action)

            last_error = info.get("error", None)
            rewards.append(f"{reward:.2f}")

            # Strict logging output format
            print(f"[STEP] step={step} action={action.target_id} reward={reward:.2f} done={str(done).lower()} error={last_error if last_error else 'null'}")
